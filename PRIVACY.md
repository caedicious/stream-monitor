...
- The desktop application sends an **anonymous install ping** to a server
  operated by the developer (`https://schedapi.caedvt.com`) on launch and
  once a day. It contains a random install ID generated by the app on
  first run, the app's version number, and the operating system name.
  That is the entire payload: no streamer names, no Twitch account
  details, no browsing data, and the server does not store your IP
  address. It exists only to count how many installs are active. You can
  turn it off at any time with the "Send anonymous install ping" checkbox
  in the desktop app's Settings. The extension never sends it.
- Apart from that ping, neither the extension nor the desktop application
  contacts any server operated by the developer or by any third party
  (other than Twitch).
...
- Apart from the anonymous install count described above, we do **not**
  use analytics, telemetry, advertising networks, or any third-party
  trackers.
...
The anonymous install ping can be turned off in the desktop app's Settings
