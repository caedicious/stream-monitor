...
const OPEN_TABS_BROWSER = "firefox";
...
async function shouldAutoMute(streamer) {
  const result = await browser.storage.local.get(["autoMute", "muteExemptStreamers"]);
...
async function shouldAutoFocus() {
  // Default ON: if a user has Firefox configured to open external tabs
  // in background (browser.tabs.loadDivertedInBackground = true), the
  // new stream tab can fail to start playing properly and Twitch may
  // not count the viewer, costing the user a streak. Auto-focusing via
  // the extension API works around the browser-level setting. Users
  // who explicitly toggle OFF in the popup get stored false and that's
  // respected.
  const { autoFocusTabs } = await browser.storage.local.get("autoFocusTabs");
  return autoFocusTabs ?? true;
}
...
async function reportOpenTabs(reason) {
  try {
    ...
    const resp = await postJson(OPEN_TABS_URL, { browser: OPEN_TABS_BROWSER, streamers, reason });
    if (resp.status === 404) {
...
// badge and the popup's "Streaks at Risk" section survive a background
// page suspend/resume cycle. Cleared when the user clicks the row
// (acknowledges) or when the entry expires (deadline + 4h buffer).
// ---------------------------------------------------------------------------

const ACK_EXPIRY_BUFFER_MS = 4 * 60 * 60 * 1000;
...
async function saveAtRiskStreaks(map) {
  await browser.storage.local.set({ atRiskStreaks: map });
}
...
async function refreshStreakBadge(mapOpt) {
  ...
  try {
    await browser.action.setBadgeBackgroundColor({ color: STREAK_BADGE_COLOR });
    await browser.action.setBadgeText({
      text: unack === 0 ? "" : unack > 9 ? "9+" : String(unack),
    });
  } catch (e) {
    // browser.action not available; non-fatal.
  }
}
